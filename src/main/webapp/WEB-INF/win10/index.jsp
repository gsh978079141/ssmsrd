...
<head>
    ...
    <script>
        function deskclick(url,name,icon){
            Win10.openUrl(url,"<img class=\"icon\" src=\""+icon+"\"/>"+name);
        }
        //S   获得桌面图标
        $.post("resource/getDesk.do", {}, function(data) {
            var desk=$("#win10-shortcuts");
            for(var i=0;i<data.length;i++){
                ...
                //         </div>
                var icon="./img/icon/"+data[i].icon;
                var url="'"+data[i].url+"'";
                var name="'"+data[i].name+"'";
                var str= "<div class=\"shortcut\" onclick=\"javascript:deskclick("+url+","+name+","+"'"+icon+"');\">"+
